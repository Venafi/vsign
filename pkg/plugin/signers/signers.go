...
// Signer go tests are in ../../../../test/e2e_test.go
...
type Signer struct {
	...
	Verify func(*os.File, options.VerifyOptions, VerifyOpts) error
	// VerifyStream is like Verify but doesn't need to seek.
	VerifyStream func(io.Reader, VerifyOpts) error
...
func ByFile(name, sigtype string) (*Signer, error) {
	...
	// Detect if XML
	/*if IsValidXML(f) {
		return ByMagic(magic.FileTypeXML), nil
	}*/
