...
func (jd *JarDigest) Sign(ctx context.Context, cert *certloader.Certificate, alias string, sectionsOnly, inlineSignature, apkV2 bool, connector endpoint.Connector) (*binpatch.PatchSet, *pkcs9.TimestampedSignature, error) {
	...
	// Sign sigfile
	sig := pkcs7.NewBuilder(cert.Signer(), cert.Chain(), jd.Hash, connector)
