...
func NewBuilder(privKey crypto.Signer, certs []*x509.Certificate, opts crypto.SignerOpts, connector endpoint.Connector) *SignatureBuilder {
...
func (sb *SignatureBuilder) Sign() (*ContentInfoSignedData, error) {
	...
	env, err := sb.tpp.GetEnvironment()
	if err != nil {
		return nil, fmt.Errorf("TPP connection error: %v", err.Error())
	}

	certs, err := c.ParseCertificates(env.CertificateChainData)
	if err != nil {
		return nil, fmt.Errorf("certificate parsing error: %v", err.Error())
	}
	...
	sig, err := sb.tpp.Sign(&endpoint.SignOption{
		KeyID:      env.KeyID,
		Mechanism:  1,
		DigestAlg:  "sha256",
		Payload:    digest,
