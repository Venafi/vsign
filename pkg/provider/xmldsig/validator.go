...
func NewValidator(xml string, opts signers.VerifyOpts) (*Validator, error) {
	...
	v := &Validator{tppOpts: opts, signatureData: signatureData{xml: doc}}
	return v, nil
}
...
func (v *Validator) ValidateReferences() ([]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("error validating references: %v", err.Error())
	}
	...
	for _, doc := range referenced {
		...
		if err != nil {
			return nil, fmt.Errorf("error writing XML doc: %v", err.Error())
		}
		ref = append(ref, docStr)
	}
	err = v.validateSignature()
	if err != nil {
		return nil, fmt.Errorf("error validating XML references: %v", err.Error())
